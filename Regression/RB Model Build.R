...
########## RB MODEL ##########
data = na.omit(RB) # put different position here 
...
### Injury Interaction
Reg <- lm(FantasyPoints ~ FantasyPointsMean_Rolling*RBInjured, data=train)
summary(Reg)

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Last5*RBInjured, data=train)
summary(Reg)

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Last3*RBInjured, data=train)
summary(Reg)

Reg <- lm(FantasyPoints ~ RushShareMean_Rolling*RBInjured, data=train)
summary(Reg)

Reg <- lm(FantasyPoints ~ RushShareMean_Last5*RBInjured, data=train)
summary(Reg)
...
# Best combination of FantasyPointsMeam and RushShareMean

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Rolling, data=train)
summary(Reg)
evaluate(Reg)

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Last3, data=train)
summary(Reg)
evaluate(Reg) # Lowest MSE

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Last5, data=train)
summary(Reg)
evaluate(Reg)

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Last3 + RushShareMean_Rolling, data=train)
summary(Reg)
evaluate(Reg)

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Last3 + RushShareMean_Last3, data=train)
summary(Reg)
evaluate(Reg)

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Last3 + RushShareMean_Last5, data=train)
summary(Reg)
evaluate(Reg)

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Last5 + RushShareMean_Rolling, data=train)
summary(Reg)
evaluate(Reg)

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Last5 + RushShareMean_Last3, data=train)
summary(Reg)
evaluate(Reg)

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Last5 + RushShareMean_Last5, data=train)
summary(Reg)
evaluate(Reg)

# Add OPR
Reg <- lm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Last3 + OPR, data=train)
summary(Reg)
evaluate(Reg) 

# Try Adding TargetShare
Reg <- lm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Last3 +TargetShareMean_Rolling + OPR, data=train)
summary(Reg)
evaluate(Reg) 

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Last3 + TargetShareMean_Last3 + OPR, data=train)
summary(Reg)
evaluate(Reg) 

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Last3 + TargetShareMean_Last5 + OPR, data=train)
summary(Reg)
evaluate(Reg) 

# Not worth adding TargetShare (not significant)

# Add Injuries
Reg <- lm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Last3 + OPR + QBInjured, data=train)
summary(Reg) # Not Sig
evaluate(Reg) 

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Last3 + OPR + RBInjured, data=train)
summary(Reg) # Sig
evaluate(Reg)

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Last3 + OPR + WRInjured, data=train)
summary(Reg) # Sig
evaluate(Reg)

Reg <- lm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Last3 + OPR + TEInjured, data=train)
summary(Reg) # Not sig
evaluate(Reg) 

# Final Model
Reg <- lm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Last3 + OPR + RBInjured + WRInjured, data=train)
summary(Reg) 
evaluate(Reg)


# Does this model perform better with Random Forest?
Forest <- randomForest(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Last3 + OPR + RBInjured + WRInjured, data=train, importance=TRUE, ntree=1000, do.trace=50)
Forest
evaluate(Forest) # No

# Does this model perform better with Gradient Boosting?
Boost <- gbm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Last3 + OPR + RBInjured + WRInjured, data=train, n.trees=1000)
summary(Boost)
evaluate(Boost) # No
...
# Only FantasyPoints, RushShares, OPR, RBInjured, WRInjured
Forest <- randomForest(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Rolling + FantasyPointsMean_Last5 + RushShareMean_Last5 + FantasyPointsMean_Last3 + RushShareMean_Last3 + OPR + RBInjured + WRInjured, data=train, importance=TRUE, ntree=1000, do.trace=50)
Forest
evaluate(Forest) # Not better than regression

Boost <- gbm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Rolling + FantasyPointsMean_Last5 + RushShareMean_Last5 + FantasyPointsMean_Last3 + RushShareMean_Last3 + OPR + RBInjured + WRInjured, data=train, n.trees=1000)
summary(Boost)
evaluate(Boost) # Not better than regression

Boost <- gbm(FantasyPoints ~ FantasyPointsMean_Rolling + RushShareMean_Rolling + FantasyPointsMean_Last5 + RushShareMean_Last5 + FantasyPointsMean_Last3 + RushShareMean_Last3 + OPR + RBInjured + WRInjured, data=train, n.trees=10000)
summary(Boost)
evaluate(Boost) # Not better than regression
...
library(stargazer)
stargazer(Reg, type='html', title='RB Regression Model')
...
write.csv(data, "~/Desktop/MGSC 434 Final Project/Optimization Data/RB2021.csv")
